...
export default async function handler(
  ...
  await page.goto('https://www.glassdoor.com/Reviews/Amazon-Reviews-E6036.htm');

  const windowObject = await page.evaluate(() => {
    return (window as any).appCache;
  });
  // save the content of the page to a file.html
...
function getEmployerReviews(query: { [key: string]: any }) {
  ...
  const employerReviewsKey = Object.keys(query).find(
    (key) => key.startsWith('employerReviews') && query[key].allReviewsCount
  );
